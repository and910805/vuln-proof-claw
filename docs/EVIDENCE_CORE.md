...
Raw evidence and canonical metadata can now be persisted transactionally and re-verified from stored bytes. Raw payloads remain internal and reports expose only their integrity status and safe metadata. The process-local Worker lifecycle validates successful Evidence references, while a concrete target-facing runtime, authenticated evidence access, and report export artifacts remain v0.1 implementation gates.
