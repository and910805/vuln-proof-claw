...
系統目前可 transactional 保存原始證據與 canonical metadata，並由資料庫內容重新驗證完整性。原始 payload 維持內部使用，報告只呈現完整性狀態與安全 metadata。持久化 Worker lifecycle registry 會驗證成功結果引用的 Evidence，並在重啟後 reconcile abandoned work；具體面向目標的 runtime、具備身分驗證的證據存取與報告匯出 artifact，仍是 v0.1 後續實作關卡。
