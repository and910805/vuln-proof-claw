...
[![Version](https://img.shields.io/badge/version-0.0.15-blue)](CHANGELOG.zh-TW.md)
...
> [!IMPORTANT]
> **Pre-alpha 狀態：** Phase 0.0.15 提供一條預設關閉、受 Scope 約束的被動 URL
...
| Execution | Opt-in DNS-pinned passive GET capture、持久化 Worker、orphan cleanup 與受限 container policy boundary；privileged Engine gateway 與任意工具仍停用 |
...
Markdown／JSON 報告匯出、經測試的 orphan-runtime cleanup 契約，以及完整的受限
container policy adapter。Evidence Core 里程碑仍待完成 privileged Engine gateway、
scope egress、Worker executor 與啟動整合。
